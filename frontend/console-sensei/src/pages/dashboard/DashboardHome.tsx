...
import { LayoutDashboard, Code2, ExternalLink, Trash2, RefreshCw, ChevronDown, ChevronUp, ChevronsLeftRight, ChevronsLeft } from "lucide-react";

const PLACEHOLDER = `<iframe title="My Power BI Report" width="1140" height="541.25" src="https://app.powerbi.com/reportEmbed?reportId=..." frameborder="0" allowFullScreen="true"></iframe>`;

const LS_SRC = "pbi_active_src";
const LS_CODE = "pbi_iframe_code";
...
export default function DashboardHome() {
  ...
  const [panelOpen, setPanelOpen] = useState<boolean>(
    () => !localStorage.getItem(LS_SRC)   // open panel only if no saved embed
  );
  ...
  const handleEmbed = () => {
    ...
    // Accept raw src URL or full iframe HTML
    ...
    <div className="flex flex-col gap-3 h-[calc(100vh-8rem)]">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-black tracking-tighter">Power BI Dashboard</h1>
            <p className="text-foreground/40 text-sm font-medium">Embed and view any Power BI report instantly</p>
          </div>
        </div>
        ...
        {activeIframe ? (
          ...
          >
            <div className="w-20 h-20 rounded-3xl bg-brand-blue/10 flex items-center justify-center">
              <LayoutDashboard className="w-9 h-9 text-brand-blue/50" />
            </div>
            <div className="text-center">
              <p className="font-black text-lg text-foreground/30">No dashboard embedded yet</p>
